...
const UpdateClassModal = ({ className, show, onHide, onSuccessCallBack }) => {
  ...
  const handleUpdateClassOptionA = () => {
    ...
    axios
      .post(
        `http://0.0.0.0:8000/class/datasets/import/`,
  ...
  const handleUpdateClassOptionB = () => {
    ...
    axios
      .post(`http://0.0.0.0:8000/class/data/add`, dataSamplesObject)
      .then((response) => {
        if (response?.status === 200) {
          alert("Class Successfully Updated!");
          onSuccessCallBack();
  ...
  return (
    <Modal show={show} onHide={onHide} size="md">
      <Modal.Header closeButton>
        <Modal.Title>{`Update Class ${className}`}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form className="mx-5">
          ...
          <div className="flex justify-center mt-2">
            <Button
              variant="primary"
              className="mt-2"
              onClick={handleUpdateClassOptionA}
            >
              Update Class
            </Button>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-center">
              <Button
                variant="primary"
                className="mt-2"
                onClick={handleUpdateClassOptionB}
              >
                Update Class
              </Button>
            </div>
